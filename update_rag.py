...
async def process_rag_update(data):
    ...
    try:
        ...
        logger.info(f"Total unique chunks to process: {len(combined_chunked_data)} ({len(existing_chunked_data)} existing, {len(new_chunked_data)} new, {added_count} actually added)")
